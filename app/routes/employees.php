...
$app->post('/empleados/eliminar', function (Request $request, Response $response) {
    ...
    $sql = "SELECT COUNT(_id) cantidad_pagos FROM pagos WHERE id_empleado = {$miEmpleado['id']} AND fecha_pago IS NOT NULL";
    $respCountRegs = $localConnection->goQuery($sql);
    $object['sql']['contar_registros_Asignados'] = $sql;
    $respPagos = $respCountRegs[0]['cantidad_pagos'];

    if (intval($respCountRegs[0]['cantidad_pagos']) == 0) {  // Eliminar empleado solo si no tiene asignaciones de tareas anteriores
        $eliminado = true;
        ...
        $sql = 'DELETE FROM empresas_usuarios WHERE id_usuario = ' . $miEmpleado['id'] . ';';
        $object['sql']['eliminar_registros'] = $sql;
        $object['sql'] = $sql;
        $object['response'] = json_encode($localConnection2->goQuery($sql));

        $sql = 'DELETE FROM empresas_usuarios_departamentos WHERE id_empleado = ' . $miEmpleado['id'] . ';';
        $object['response'] = json_encode($localConnection2->goQuery($sql));
    } else {
        $eliminado = false;
        // Dehabilitar el empleado sin eliminarlo
        $sql = "UPDATE empresas_usuarios SET activo = 0 WHERE id_usuario = {$miEmpleado['id']}";
        $object['response_desactivar'] = json_encode($localConnection2->goQuery($sql));
    }

    if ($eliminado) {
        $object['message'] = "El empleado ha sido eliminado";
    } else {
        $object['message'] = "El empleado ha sido deshabilitado, puede consultar el historial de este enpleado o habilitarlo de nuevo posteriormente desde la opción 'Activación de Empleados' del menú Empleados";
    }

    $object['eliinado'] = $eliminado;
