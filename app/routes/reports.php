        ...
        try {
            ...
            if (DB_DRIVER === 'pgsql') {
                $sqlTasks = "SELECT
                    ...
                    (CASE WHEN COUNT(ldea.id_empleado) = COUNT(ldea.fecha_terminado) THEN MAX(ldea.fecha_terminado) ELSE NULL END) AS fecha_terminado,
                    TO_CHAR(MIN(ldea.fecha_inicio), 'DD/MM/YYYY HH12:MI AM') AS fecha_inicio_formateada,
                    TO_CHAR((CASE WHEN COUNT(ldea.id_empleado) = COUNT(ldea.fecha_terminado) THEN MAX(ldea.fecha_terminado) ELSE NULL END), 'DD/MM/YYYY HH12:MI AM') AS fecha_terminado_formateada,
                    TO_CHAR(MIN(ldea.fecha_inicio), 'DD/MM/YYYY HH12:MI AM') AS fecha_estimada_inicio_formateada,
                    TO_CHAR((CASE WHEN COUNT(ldea.id_empleado) = COUNT(ldea.fecha_terminado) THEN MAX(ldea.fecha_terminado) ELSE NULL END), 'DD/MM/YYYY HH12:MI AM') AS fecha_estimada_fin_formateada,
                    'terminado' AS status,
                ...
                $sqlTasks = "SELECT
                    ...
                    DATE_FORMAT((CASE WHEN COUNT(ldea.id_empleado) = COUNT(ldea.fecha_terminado) THEN MAX(ldea.fecha_terminado) ELSE NULL END), '%d/%m/%Y %h:%i %p') AS fecha_terminado_formateada,
                    DATE_FORMAT(MIN(ldea.fecha_inicio), '%d/%m/%Y %h:%i %p') AS fecha_estimada_inicio_formateada,
                    DATE_FORMAT((CASE WHEN COUNT(ldea.id_empleado) = COUNT(ldea.fecha_terminado) THEN MAX(ldea.fecha_terminado) ELSE NULL END), '%d/%m/%Y %h:%i %p') AS fecha_estimada_fin_formateada,
                    'terminado' AS status,
                    COALESCE(COUNT(DISTINCT ldea.id_empleado), 1) as cant_empleados,
                    (
                        SELECT COALESCE(SUM(ptp.tiempo * op_sub.cantidad), 0)
