...
return function (App $app) {
  ...
  $app->post('/orden/abono', function (Request $request, Response $response, $args) {
    ...
    try {
      ...
      // Auditoría de seguridad 2026-09-10: addslashes() NO protege contra
      // inyección SQL en esta base de datos -- Postgres corre con
      // standard_conforming_strings=on, así que el backslash de
      // addslashes() no escapa nada dentro de un literal '...'. Confirmado
      // en vivo (SELECT ... WHERE '1'='1\'; DROP TABLE ...; --' ejecutó el
      // DROP). Todo lo que sigue en este endpoint pasa por parámetros
      // preparados (?), nunca concatenado directo al SQL.
      ...
      $sql = 'INSERT INTO abonos(moment, id_orden, abono, descuento, nota_credito, id_empleado, detalle) VALUES (?, ?, ?, ?, ?, ?, ?)';
      $resultAbono = $localConnection->goQuery($sql, [
        $now,
        intval($datosAbono['id']),
        $abono_val,
        $descuento_val,
        $nota_credito_val,
        intval($datosAbono['empleado']),
        $detalleFinal,
      ]);
      ...
      $detalleAbonoOrden = 'Abono a Orden #' . intval($datosAbono['id']);
      ...
      if (!empty($pagosGenericos)) {
        ...
        foreach ($pagosGenericos as $pago) {
          ...
          $tipoAbono = $datosAbono['tipoAbono'] ?? '';
        ...
        $tipoAbonoLegado = $datosAbono['tipoAbono'] ?? '';

        if (floatval($datosAbono['montoDolaresEfectivo'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Efectivo');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, 'Dólares', 'Efectivo', ?, '1', ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, intval($datosAbono['id']), floatval($datosAbono['montoDolaresEfectivo']), $idMoneda, $idMetodo]);
          $sql = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Dólares', 1, ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [floatval($datosAbono['montoDolaresEfectivo']), $tipoAbonoLegado, $responsable, $detalleAbonoOrden, $idMoneda]);
        }

        if (floatval($datosAbono['montoDolaresZelle'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Zelle');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, detalle, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, ?, 'Dólares', 'Zelle', ?, '1', ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, $datosAbono['detalleZelle'] ?? '', intval($datosAbono['id']), floatval($datosAbono['montoDolaresZelle']), $idMoneda, $idMetodo]);
        }

        if (floatval($datosAbono['montoDolaresPanama'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Panamá');
          $sql = "INSERT INTO metodos_de_pago (detalle, tipo_de_pago, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, ?, 'Dólares', 'Panamá', ?, '1', ?, ?)";
          $localConnection->goQuery($sql, [$datosAbono['detallePanama'] ?? '', $tipoAbonoLegado, intval($datosAbono['id']), floatval($datosAbono['montoDolaresPanama']), $idMoneda, $idMetodo]);
        }

        if (floatval($datosAbono['montoPesosEfectivo'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Efectivo');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, 'Pesos', 'Efectivo', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, intval($datosAbono['id']), floatval($datosAbono['montoPesosEfectivo']), $tasa_peso, $idMoneda, $idMetodo]);
          $sql = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Pesos', ?, ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [floatval($datosAbono['montoPesosEfectivo']), $tasa_peso, $tipoAbonoLegado, $responsable, $detalleAbonoOrden, $idMoneda]);
        }

        if (floatval($datosAbono['montoPesosTransferencia'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Transferencia');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, detalle, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, ?, 'Pesos', 'Transferencia', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, $datosAbono['detallePesosTransferencia'] ?? '', intval($datosAbono['id']), floatval($datosAbono['montoPesosTransferencia']), $tasa_peso, $idMoneda, $idMetodo]);
        }

        if (floatval($datosAbono['montoBolivaresEfectivo'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Efectivo');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, 'Bolívares', 'Efectivo', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, intval($datosAbono['id']), floatval($datosAbono['montoBolivaresEfectivo']), $tasa_dolar, $idMoneda, $idMetodo]);
          $sql = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Bolívares', ?, ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [floatval($datosAbono['montoBolivaresEfectivo']), $tasa_dolar, $tipoAbonoLegado, $responsable, $detalleAbonoOrden, $idMoneda]);
        }

        if (floatval($datosAbono['montoBolivaresPunto'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Punto');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, 'Bolívares', 'Punto', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, intval($datosAbono['id']), floatval($datosAbono['montoBolivaresPunto']), $tasa_dolar, $idMoneda, $idMetodo]);
        }

        if (floatval($datosAbono['montoBolivaresPagomovil'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Pagomovil');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, detalle, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, ?, 'Bolívares', 'Pagomovil', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, $datosAbono['detallePagomovil'] ?? '', intval($datosAbono['id']), floatval($datosAbono['montoBolivaresPagomovil']), $tasa_dolar, $idMoneda, $idMetodo]);
        }

        if (floatval($datosAbono['montoBolivaresTransferencia'] ?? 0) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Transferencia');
          $sql = "INSERT INTO metodos_de_pago (tipo_de_pago, detalle, id_orden, moneda, metodo_pago, monto, tasa, id_moneda, id_metodo_pago) VALUES (?, ?, ?, 'Bolívares', 'Transferencia', ?, ?, ?, ?)";
          $localConnection->goQuery($sql, [$tipoAbonoLegado, $datosAbono['detalleBolivaresTransferencia'] ?? '', intval($datosAbono['id']), floatval($datosAbono['montoBolivaresTransferencia']), $tasa_dolar, $idMoneda, $idMetodo]);
        }
      }
  ...
  $app->post('/ordenes/nueva/custom/edit', function (Request $request, Response $response, $arg) {
    ...
    if (!empty($pagosGenericos)) {
      ...
      foreach ($pagosGenericos as $pago) {
        ...
        $object['metodos_pago_response'][] = insertarMetodoPagoGenerico($localConnection, $id_orden_a_editar, 'Abono a Orden', $metodo, $monto, $detalle, $tasa);

        if ((int) $metodo['es_efectivo'] === 1) {
          $object['metodos_pago_response'][] = insertarCajaGenerico($localConnection, $monto, $metodo, $tasa, 'Abono a Orden', $arr['responsable'], "Abono a Orden #{$id_orden_a_editar}");
        }
      }
    } else {
      // CAMINO LEGADO: formularios aún no migrados al componente dinámico
      // (Fase 7, en curso) -- sin cambios de comportamiento.
      // Auditoría de seguridad 2026-09-10: este bloque usaba addslashes(),
      // que no protege nada contra esta base de datos (Postgres corre con
      // standard_conforming_strings=on) -- mismo hallazgo y mismo fix que
      // POST /orden/abono, ver memoria de seguridad. Reescrito a parámetros
      // preparados (?), sin cambiar el comportamiento funcional.
      $idOrdenEditar = intval($id_orden_a_editar);
      $responsableAbono = intval($arr['responsable']);
      $detalleAbonoOrdenEdit = "Abono a Orden #{$idOrdenEditar}";

      if (floatval($newJson['montoDolaresEfectivo'] ?? 0) > 0) {
        $monto = floatval($newJson['montoDolaresEfectivo']);
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Efectivo');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Dólares', 'Efectivo', ?, '1', '', ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $idMoneda, $idMetodo]);
        $sql_metodos_pago = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Dólares', 1, 'Abono a Orden', ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$monto, $responsableAbono, $detalleAbonoOrdenEdit, $idMoneda]);
      }

      if (floatval($newJson['montoDolaresZelle'] ?? 0) > 0) {
        $monto = floatval($newJson['montoDolaresZelle']);
        $detalle = $newJson['montoDolaresZelleDetalle'] ?? '';
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Zelle');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Dólares', 'Zelle', ?, '1', ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $detalle, $idMoneda, $idMetodo]);
      }

      if (floatval($newJson['montoDolaresPanama'] ?? 0) > 0) {
        $monto = floatval($newJson['montoDolaresPanama']);
        $detalle = $newJson['montoDolaresPanamaDetalle'] ?? '';
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Panamá');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Dólares', 'Panamá', ?, '1', ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $detalle, $idMoneda, $idMetodo]);
      }
      ...
      if (floatval($newJson['montoPesosEfectivo'] ?? 0) > 0) {
        ...
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Efectivo');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Pesos', 'Efectivo', ?, ?, '', ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $idMoneda, $idMetodo]);
        $sql_metodos_pago = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Pesos', ?, 'Abono a Orden', ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$monto, $tasa, $responsableAbono, $detalleAbonoOrdenEdit, $idMoneda]);
      }

      if (floatval($newJson['montoPesosTransferencia'] ?? 0) > 0) {
        $monto = floatval($newJson['montoPesosTransferencia']);
        $tasa = floatval($newJson['tasa_peso'] ?? 1);
        $detalle = $newJson['montoPesosTransferenciaDetalle'] ?? '';
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Transferencia');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Pesos', 'Transferencia', ?, ?, ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $detalle, $idMoneda, $idMetodo]);
      }
      ...
      if (floatval($newJson['montoBolivaresEfectivo'] ?? 0) > 0) {
        ...
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Efectivo');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Bolívares', 'Efectivo', ?, ?, '', ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $idMoneda, $idMetodo]);
        $sql_metodos_pago = "INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, 'Bolívares', ?, 'Abono a Orden', ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$monto, $tasa, $responsableAbono, $detalleAbonoOrdenEdit, $idMoneda]);
      }
      ...
      if (floatval($newJson['montoBolivaresPunto'] ?? 0) > 0) {
        ...
        $tasa = floatval($newJson['tasa_dolar'] ?? 1);
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Punto');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Bolívares', 'Punto', ?, ?, '', ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $idMoneda, $idMetodo]);
      }
      ...
      if (floatval($newJson['montoBolivaresPagomovil'] ?? 0) > 0) {
        ...
        $tasa = floatval($newJson['tasa_dolar'] ?? 1);
        $detalle = $newJson['montoBolivaresPagomovilDetalle'] ?? '';
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Pagomovil');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Bolívares', 'Pagomovil', ?, ?, ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $detalle, $idMoneda, $idMetodo]);
      }
      ...
      if (floatval($newJson['montoBolivaresTransferencia'] ?? 0) > 0) {
        ...
        $tasa = floatval($newJson['tasa_dolar'] ?? 1);
        $detalle = $newJson['montoBolivaresTransferenciaDetalle'] ?? '';
        list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Transferencia');
        $sql_metodos_pago = "INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, 'Bolívares', 'Transferencia', ?, ?, ?, ?, ?)";
        $object['metodos_pago_response'][] = $localConnection->goQuery($sql_metodos_pago, [$idOrdenEditar, $monto, $tasa, $detalle, $idMoneda, $idMetodo]);
      }
    }
...
$object['sales_commission_ISSET'][] = false;
      ...
      if (!empty($pagosGenericos)) {
        ...
        // CAMINO LEGADO: formularios aún no migrados al componente dinámico
        // (Fase 7, en curso). Los 9 bloques de abajo interpolaban el monto/tasa
        // del request directo en el SQL (solo `floatval()` en el `if`, nunca en
        // el valor insertado) -- inyección SQL real en la creación de orden,
        // el flujo de negocio más sensible del sistema (auditoría de seguridad
        // 2026-09-09). Convertidos a parámetros (?), sin cambiar ningún otro
        // comportamiento (mismos literales de moneda/método, mismo dual-write
        // de id_moneda/id_metodo_pago).
        $sqlMetodoPago = 'INSERT INTO metodos_de_pago (id_orden, moneda, metodo_pago, monto, tasa, detalle, id_moneda, id_metodo_pago) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
        $sqlCaja = 'INSERT INTO caja (monto, moneda, tasa, tipo, id_empleado, detalle, id_moneda) VALUES (?, ?, ?, ?, ?, ?, ?)';

        if (floatval($arr['montoDolaresEfectivo']) > 0) {  // Usar floatval para comparar con 0
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Efectivo');
          $monto = floatval($arr['montoDolaresEfectivo']);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Dólares', 'Efectivo', $monto, 1, '', $idMoneda, $idMetodo]);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlCaja, [$monto, 'Dólares', 1, 'Orden Nueva', $newJson['responsable'], 'Nueva Orden', $idMoneda]);
        }

        if (floatval($arr['montoDolaresZelle']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Zelle');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Dólares', 'Zelle', floatval($arr['montoDolaresZelle']), 1, $arr['montoDolaresZelleDetalle'] ?? '', $idMoneda, $idMetodo]);
        }

        if (floatval($arr['montoDolaresPanama']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Dólares', 'Panamá');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Dólares', 'Panamá', floatval($arr['montoDolaresPanama']), 1, $arr['montoDolaresPanamaDetalle'] ?? '', $idMoneda, $idMetodo]);
        }

        if (floatval($arr['montoPesosEfectivo']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Efectivo');
          $monto = floatval($arr['montoPesosEfectivo']);
          $tasaPeso = floatval($arr['tasa_peso']);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Pesos', 'Efectivo', $monto, $tasaPeso, '', $idMoneda, $idMetodo]);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlCaja, [$monto, 'Pesos', $tasaPeso, 'Orden Nueva', $newJson['responsable'], 'Nueva Orden', $idMoneda]);
        }

        if (floatval($arr['montoPesosTransferencia']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Pesos', 'Transferencia');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Pesos', 'Transferencia', floatval($arr['montoPesosTransferencia']), floatval($arr['tasa_peso']), $arr['montoPesosTransferenciaDetalle'] ?? '', $idMoneda, $idMetodo]);
        }

        if (floatval($arr['montoBolivaresEfectivo']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Efectivo');
          $monto = floatval($arr['montoBolivaresEfectivo']);
          $tasaDolar = floatval($arr['tasa_dolar']);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Bolívares', 'Efectivo', $monto, $tasaDolar, '', $idMoneda, $idMetodo]);
          $object['metodos_pago'][] = $localConnection->goQuery($sqlCaja, [$monto, 'Bolívares', $tasaDolar, 'Orden Nueva', $newJson['responsable'], 'Nueva Orden', $idMoneda]);
        }

        if (floatval($arr['montoBolivaresPunto']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Punto');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Bolívares', 'Punto', floatval($arr['montoBolivaresPunto']), floatval($arr['tasa_dolar']), '', $idMoneda, $idMetodo]);
        }

        if (floatval($arr['montoBolivaresPagomovil']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Pagomovil');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Bolívares', 'Pagomovil', floatval($arr['montoBolivaresPagomovil']), floatval($arr['tasa_dolar']), $arr['montoBolivaresPagomovilDetalle'] ?? '', $idMoneda, $idMetodo]);
        }

        if (floatval($arr['montoBolivaresTransferencia']) > 0) {
          list($idMoneda, $idMetodo) = resolverIdsMonedaMetodo($localConnection, 'Bolívares', 'Transferencia');
          $object['metodos_pago'][] = $localConnection->goQuery($sqlMetodoPago, [$last_id, 'Bolívares', 'Transferencia', floatval($arr['montoBolivaresTransferencia']), floatval($arr['tasa_dolar']), $arr['montoBolivaresTransferenciaDetalle'] ?? '', $idMoneda, $idMetodo]);
        }
      }
...
id_products_attributes, id_size, id_tela, moment
  ...
  if (!empty($pagosGenericos)) {
    ...
    // diferencia del camino legado de esta ruta (que nunca escribía en
    // `caja`, un vacío de la implementación original sin uso histórico real),
    // este camino sí escribe en `caja` para métodos es_efectivo=1, igual que
    // el resto de endpoints de creación de orden -- consistente con el resto
    // del sistema, no una reproducción de la limitación original.
    foreach ($pagosGenericos as $pago) {
      $monto = floatval($pago['monto'] ?? 0);
      if ($monto <= 0) {
        continue;
      ...
      insertarMetodoPagoGenerico($localConnection, $id_orden, 'Orden nueva', $metodo, $monto, $detalle, $tasa);

      if ((int) $metodo['es_efectivo'] === 1) {
        insertarCajaGenerico($localConnection, $monto, $metodo, $tasa, 'Orden Nueva', $data['responsable'] ?? 0, 'Nueva Orden (desde presupuesto)');
      }
    }
  } else {
    // CAMINO LEGADO: formularios aún no migrados al componente dinámico
    // (Fase 7, en curso). Corregido en un ajuste previo de la Fase 6: esta
    // ruta escribía 'Dolares'/'Bolivares'/'Banesco Panama' sin tilde
    // (fragmentación de datos preexistente, verificado 0 filas históricas con
    // esa grafía) -- alineado a los mismos strings exactos que usa el resto
    // del sistema.
    ...
    if (isset($data['montoPesosPago']) && $data['montoPesosPago'] > 0) {
      ...
      $localConnection->goQuery($sql_metodo_pago, [$id_orden, 'Pesos', 'Pago movil', $detalle, 'Orden nueva', floatval($data['montoPesosPago']), $tasa_peso, $now, $idMoneda, $idMetodo]);
    }
