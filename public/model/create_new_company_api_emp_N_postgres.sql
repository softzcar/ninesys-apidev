...
-- Catálogo de monedas de la empresa (Fase 2 del rediseño de monedas/métodos de pago).
-- id_moneda_soportada referencia monedas_soportadas_por_pais en la base compartida
-- api_empresas (cross-DB, sin FK real). Vacío por defecto: una empresa nueva se
-- configura desde el Gestor de Monedas / Wizard (Fase 5), no con datos especulativos.
CREATE TABLE catalogo_monedas (
  _id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY ,
  id_moneda_soportada INTEGER DEFAULT NULL ,
...
CREATE TABLE catalogo_metodos_pago (
  ...
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (_id),
  CONSTRAINT fk_metodos_pago_moneda FOREIGN KEY (id_moneda) REFERENCES catalogo_monedas (_id) ON UPDATE CASCADE ON DELETE RESTRICT
);

-- Registra cada período en que una moneda fue la base de la empresa (hasta
-- IS NULL = vigente). Permite saber, para cualquier transacción histórica
-- (por su columna `moment`), cuál era la moneda base en ese momento, sin
-- tener que agregar esa columna a cada tabla de transacciones.
CREATE TABLE historial_moneda_base (
  _id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY ,
  id_moneda INTEGER NOT NULL ,
  desde timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  hasta timestamp DEFAULT NULL ,
  PRIMARY KEY (_id),
  CONSTRAINT fk_historial_moneda_base_moneda FOREIGN KEY (id_moneda) REFERENCES catalogo_monedas (_id) ON UPDATE CASCADE ON DELETE RESTRICT
);
...
CREATE TABLE caja (
  ...
  moneda varchar(100) NOT NULL DEFAULT '0' ,
  id_moneda INTEGER DEFAULT NULL REFERENCES catalogo_monedas (_id) ON UPDATE CASCADE ON DELETE RESTRICT,
...
-- Fase 8 del rediseño de monedas: enfoque híbrido -- dolares/pesos/bolivares
-- arriba se quedan como están (cero riesgo sobre saldo/cierre histórico),
-- estas 2 tablas hijas cubren cualquier moneda adicional del catálogo real.
CREATE TABLE caja_cierres_extra (
  _id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  id_caja_cierres INTEGER NOT NULL,
  id_moneda INTEGER NOT NULL,
  monto decimal(14, 2) NOT NULL DEFAULT 0,
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (_id),
  FOREIGN KEY (id_caja_cierres) REFERENCES caja_cierres(_id) ON UPDATE CASCADE ON DELETE CASCADE,
  FOREIGN KEY (id_moneda) REFERENCES catalogo_monedas(_id) ON UPDATE CASCADE ON DELETE RESTRICT
);
CREATE INDEX idx_caja_cierres_extra_cierre ON caja_cierres_extra(id_caja_cierres);
CREATE TABLE caja_fondos_extra (
  _id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  id_caja_fondos INTEGER NOT NULL,
  id_moneda INTEGER NOT NULL,
  monto decimal(14, 2) NOT NULL DEFAULT 0,
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (_id),
  FOREIGN KEY (id_caja_fondos) REFERENCES caja_fondos(_id) ON UPDATE CASCADE ON DELETE CASCADE,
  FOREIGN KEY (id_moneda) REFERENCES catalogo_monedas(_id) ON UPDATE CASCADE ON DELETE RESTRICT
);
CREATE INDEX idx_caja_fondos_extra_fondo ON caja_fondos_extra(id_caja_fondos);
...
CREATE TABLE servicios_maquinas (
  ...
  tipo_servicio citext NOT NULL,
  descripcion citext DEFAULT NULL,
  tecnico citext DEFAULT NULL,
...
CREATE TABLE config (
  ...
  wizard_operativo_admin SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_empresa SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_monedas SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_horario SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_personalizacion SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_gastos SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_departamentos SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_empleados SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_categorias SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_productos SMALLINT NOT NULL DEFAULT 0,
  ...
  wizard_operativo_completo SMALLINT NOT NULL DEFAULT 0,
  wizard_operativo_omitido_en TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (_id)
);
INSERT INTO config (
    _id,
...
CREATE TABLE customers (
  ...
  first_name citext DEFAULT NULL,
  last_name citext DEFAULT NULL,
  username varchar(60) DEFAULT NULL,
  cedula citext DEFAULT NULL,
...
CREATE TABLE lotes_detalles_empleados_productos (
  _id INTEGER NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  id_lotes_detalles_empleados_asignados INTEGER NOT NULL ,
  id_ordenes_productos INTEGER NOT NULL ,
...
CREATE TABLE metodos_de_pago (
  ...
  id_metodo_pago INTEGER DEFAULT NULL REFERENCES catalogo_metodos_pago (_id) ON UPDATE CASCADE ON DELETE RESTRICT,
  ...
  id_empleado_verifica INTEGER DEFAULT NULL,
  moment_verificacion timestamp DEFAULT NULL,
  PRIMARY KEY (_id)
);
CREATE INDEX idx_met_pago_verificado ON metodos_de_pago(verificado);
CREATE TABLE ordenes (
  ...
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ,
  moment_entregada timestamp without time zone DEFAULT NULL,
  PRIMARY KEY (_id)
);

-- Mantiene moment_entregada sincronizada con el status de la orden: se fija
-- automáticamente al pasar a 'entregada' (o al nacer ya en ese estado), y se
-- limpia si el status se revierte a cualquier otro valor (ej. corrección de
-- un error humano). Cubre cualquier INSERT/UPDATE presente o futuro sobre
-- ordenes.status sin depender de que cada endpoint lo sincronice a mano.
CREATE OR REPLACE FUNCTION sync_moment_entregada() RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.status = 'entregada' THEN
      NEW.moment_entregada := CURRENT_TIMESTAMP;
    END IF;
  ELSIF TG_OP = 'UPDATE' THEN
    IF NEW.status = 'entregada' AND OLD.status IS DISTINCT FROM 'entregada' THEN
      NEW.moment_entregada := CURRENT_TIMESTAMP;
    ELSIF NEW.status IS DISTINCT FROM 'entregada' AND OLD.status = 'entregada' THEN
      NEW.moment_entregada := NULL;
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_sync_moment_entregada
BEFORE INSERT OR UPDATE ON ordenes
FOR EACH ROW EXECUTE FUNCTION sync_moment_entregada();
...
CREATE TABLE pagos (
  ...
  id_lotes_detalles INTEGER DEFAULT NULL ,
  id_empleado INTEGER DEFAULT NULL ,
  cantidad numeric(6, 1) DEFAULT NULL ,
  monto_pago decimal(12, 2) DEFAULT NULL ,
  comision decimal(6, 3) NOT NULL DEFAULT 0.00 ,
...
-- Blindaje contra pagos duplicados por lote (hallazgo real 2026-09-18):
-- hace estructuralmente imposible insertar dos veces el mismo pago para el
-- mismo lote+tipo de pago, sin importar el endpoint ni la concurrencia.
-- Partido en dos indices porque UNIQUE trata cada NULL como distinto -- sin
-- esto, todas las filas con id_reposicion IS NULL (el caso real que causo
-- el bug) quedarian sin proteccion.
CREATE UNIQUE INDEX pagos_lote_detalle_unico_sin_reposicion
  ON pagos (id_lotes_detalles, detalle)
  WHERE id_lotes_detalles IS NOT NULL AND id_reposicion IS NULL;
CREATE UNIQUE INDEX pagos_lote_detalle_unico_con_reposicion
  ON pagos (id_lotes_detalles, detalle, id_reposicion)
  WHERE id_lotes_detalles IS NOT NULL AND id_reposicion IS NOT NULL;
...
CREATE TABLE products (
  ...
  comision decimal(8, 3) DEFAULT 0.00 ,
...
CREATE TABLE tintas (
  ...
  id_color_tinta INTEGER NOT NULL ,
  cantidad decimal(7, 2) NOT NULL DEFAULT 0.00 ,
  es_estimado SMALLINT NOT NULL DEFAULT 0 ,
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (_id)
);
CREATE TABLE tintas_calibracion_colores (
  ...
  id_catalogo_impresora INTEGER NOT NULL ,
  id_color_tinta INTEGER NOT NULL ,
  ml_por_metro decimal(6, 2) NOT NULL ,
  moment timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (_id),
  UNIQUE (id_catalogo_impresora, id_color_tinta)
);
...
ALTER TABLE lotes_detalles_empleados_productos ADD CONSTRAINT ldep_ibfk_1 FOREIGN KEY (id_lotes_detalles_empleados_asignados) REFERENCES lotes_detalles_empleados_asignados (_id) ON DELETE CASCADE ON UPDATE CASCADE, ADD CONSTRAINT ldep_ibfk_2 FOREIGN KEY (id_ordenes_productos) REFERENCES ordenes_productos (_id) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE tintas_calibracion_colores ADD CONSTRAINT tintas_calib_ibfk_1 FOREIGN KEY (id_catalogo_impresora) REFERENCES catalogo_impresoras (_id) ON DELETE CASCADE ON UPDATE CASCADE, ADD CONSTRAINT tintas_calib_ibfk_2 FOREIGN KEY (id_color_tinta) REFERENCES catalogo_colores_tintas (_id) ON DELETE CASCADE ON UPDATE CASCADE;
...
CREATE INDEX idx_lotes_detalles_empleados_productos_id_ldea ON lotes_detalles_empleados_productos (id_lotes_detalles_empleados_asignados);
CREATE INDEX idx_lotes_detalles_empleados_productos_id_op ON lotes_detalles_empleados_productos (id_ordenes_productos);
...
SELECT setval(pg_get_serial_sequence('config', '_id'), COALESCE((SELECT MAX(_id) FROM config), 1));
SELECT setval(pg_get_serial_sequence('customers', '_id'), COALESCE((SELECT MAX(_id) FROM customers), 1));
SELECT setval(pg_get_serial_sequence('departamentos', '_id'), COALESCE((SELECT MAX(_id) FROM departamentos), 1));
SELECT setval(pg_get_serial_sequence('products', '_id'), COALESCE((SELECT MAX(_id) FROM products), 1));
SELECT setval(pg_get_serial_sequence('products_comisiones', '_id'), COALESCE((SELECT MAX(_id) FROM products_comisiones), 1));
